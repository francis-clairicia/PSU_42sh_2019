...
Test(setenv_builtin_command, add_var_to_environment_and_can_modify_it)
{
    ...
    cr_expect_eq(eval_exec_cmd("setenv USER user", shell), 0);
    cr_expect_eq(eval_exec_cmd("setenv HOSTNAME localhost", shell), 0);
    cr_expect_eq(eval_exec_cmd("setenv VAR", shell), 0);
    ...
    cr_expect_eq(eval_exec_cmd("setenv USER name", shell), 0);
...
Test(setenv_builtin_command, print_error_if_not_start_with_letter)
{
    ...
    cr_redirect_stderr();
    cr_expect_eq(eval_exec_cmd("setenv 2VAR user", shell), -1);
...
Test(setenv_builtin_command, print_error_if_var_is_non_alphanumeric_string)
{
    ...
    cr_redirect_stderr();
    cr_expect_eq(eval_exec_cmd("setenv VAR5:2 user", shell), -1);
    cr_expect_stderr_eq_str(err);
    destroy_shell_struct(shell);
}
...
Test(setenv_builtin_command, print_environment_when_no_args_is_given)
{
    ...
    cr_redirect_stdout();
    cr_expect_eq(eval_exec_cmd("setenv", shell), 0);
...
Test(setenv_builtin_command, print_error_when_too_many_args_are_given)
{
    ...
    cr_redirect_stderr();
    cr_expect_eq(eval_exec_cmd("setenv VAR value1 value2", shell), -1);
    cr_expect_stderr_eq_str(err);
    destroy_shell_struct(shell);
}